...
用户态文件系统的设计涉及多个层次的技术选型：从内核与用户空间的交互机制，
到底层存储引擎的数据组织方式，再到文件操作语义的正确性保证和并发访问控制。
本章围绕 RucksFS 所依赖的四项关键技术展开讨论——FUSE 用户态文件系统框架、
RocksDB 存储引擎、POSIX 文件系统语义，以及悲观并发控制机制。
每一项技术的原理与工程特性将直接影响后续章节中系统架构和实现方案的选择。
...
实验结果表明，相比内核原生文件系统，FUSE 引入的额外延迟因操作类型而异。
...
针对这些瓶颈，工程实践中存在若干优化手段：

第一，利用内核侧的属性缓存（\texttt{attr\_timeout}）和目录项缓存
（\texttt{entry\_timeout}）减少对用户态的回调次数。对于单机文件系统，
不存在多客户端缓存一致性问题，缓存时长可以设置为秒级甚至更长。

第二，启用 writeback 缓存模式，让内核页缓存合并多次小写入后再批量下发，
将多次上下文切换减少为一次。

第三，使用 \texttt{splice} 系统调用在内核缓冲区和用户态之间零拷贝传输数据，
减少大文件读写的内存拷贝开销。

第四，调整 \texttt{max\_background}（后台请求上限，默认 12）和
\texttt{congestion\_threshold}（拥塞阈值，默认 9）参数，
控制内核排队行为以匹配用户态处理能力。
...
截至 2026 年，RocksDB 被广泛用于 MySQL（MyRocks）、TiKV、CockroachDB
等系统的底层存储。
...
进入 Level-0 层。与此同时，一个新的空 MemTable 立即可用于接收后续写入。
...
本章介绍了 RucksFS 设计与实现所依赖的四项基础技术。
FUSE 框架提供了将文件系统逻辑放在用户空间运行的能力，降低了开发复杂度，
代价是每次操作引入额外的用户/内核态切换开销——元数据操作的退化尤为明显。
RocksDB 基于 LSM-tree 提供写入友好的持久化存储，
Column Family 机制允许对不同类型的元数据分别配置，
WriteBatch 和 TransactionDB 为原子性和并发控制提供了底层支撑。
POSIX 语义规范定义了文件系统操作的正确性约束，
特别是 \texttt{rename} 的原子性要求和 \texttt{unlink} 的链接计数管理，
直接影响存储层的事务设计。
悲观并发控制通过先加锁再操作的方式，配合 RocksDB 的键级事务机制，
为消除元数据操作中的 TOCTOU 竞态问题提供了可靠的方案。
后续章节将以这些技术为基础，阐述 RucksFS 的具体设计方案和关键模块实现。
