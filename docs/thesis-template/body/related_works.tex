...
唤醒阻塞的应用线程，将结果返回给调用者。
图~\ref{fig:fuse_architecture} 展示了这一架构。
...
\begin{figure}[htbp]
    ...
    \node[box, fill=black!6]  (pending)    at (0, 0)   {待处理队列\\[-2pt]\footnotesize Pending};
    \node[box, fill=black!6]  (processing) at (4.5, 0) {处理中队列\\[-2pt]\footnotesize Processing};
    \node[box, fill=black!6]  (complete)   at (9, 0)   {完成\\[-2pt]\footnotesize Complete};

    \draw[arr] (pending)    -- (processing) node[midway, above, lbl] {守护进程读取};
    \draw[arr] (processing) -- (complete)   node[midway, above, lbl] {守护进程写回};

    \node[lbl, text width=5cm, align=center] at (4.5, -1.2)
        {每个请求携带唯一 ID，内核据此匹配响应并唤醒等待线程};

    \end{tikzpicture}
    \caption{FUSE 请求生命周期。内核将 VFS 请求放入待处理队列，守护进程通过 \texttt{read()} 取出后转入处理中队列，处理完毕通过 \texttt{write()} 写回响应，内核匹配请求 ID 后标记完成。}
