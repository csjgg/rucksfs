...
客户端内部的协调层是 \texttt{VfsCore}。它对 FUSE 暴露统一的 \texttt{VfsOps} 接口，同时持有 \texttt{MetadataOps} 和 \texttt{DataOps} 两侧的句柄，负责决定一次请求应该落到哪个服务、是否需要跨服务串联，以及结果返回之后是否还需要补充后续动作。相对于单纯的 RPC 转发器，它更接近整条路径上的编排层。
...
这样安排的意图较为明确：命名空间和 inode 状态的语义约束在 MetadataServer 内，字节读写、截断、清理的工作留在 DataServer 内，跨两侧的复合语义由客户端显式协调。三者的边界因此较为清晰，客户端充当 MetadataServer 元数据事务和 DataServer 字节读写结果的汇合点，把两侧的结果组合成用户看到的 POSIX 行为。代价是客户端比本地文件系统中通常只做请求转发的客户端更复杂一些，这是分离式架构必须承担的一层成本。
...
但这里也需要指出当前实现仍有一处短板。每个 inode 都预留 \texttt{max\_file\_size} 大小的逻辑地址空间，当前默认 128\,MiB，在大量小文件场景下空间利用率较低；\texttt{delete\_data} 目前仍为空操作，对象删除后物理空间不会立即回收。空间管理本身并非不重要，只是本文在工程上有意把这些复杂度推迟到后续扩展中，用来换取当前原型在数据路径上的可解释性和实现简洁性。
...
在用户态文件系统中，一项操作的可见性能很大程度上取决于内核如何缓存它的元数据。因此在进入实验之前，需要先把 FUSE 层的几项可见语义讲清楚，否则第~\ref{sec:experiments}~章中的数据容易被误读。本文主要控制三项：目录项缓存、属性缓存，以及目录遍历接口的选择。

目录项缓存（entry cache）决定内核认为“名字到 inode”的映射在多长时间内有效。属性缓存（attr cache）决定 \texttt{stat}、\texttt{getattr} 等请求是否需要到达服务端。本文当前默认将两者的 TTL 均设为 0，即每次 \texttt{lookup} 和 \texttt{getattr} 都穿透 FUSE 送到 MetadataServer；这一选择直接服务于第~\ref{sec:experiments}~章的测量目标——将元数据路径的吞吐归因到 MetadataServer 本身，而不是被客户端缓存遮蔽。在真实生产部署下，这两项 TTL 可以按需放宽到秒级，以降低重复查询的 RPC 开销。目录遍历接口显式启用 \texttt{readdirplus}，让目录遍历在返回目录项的同时附带基本属性，从而省去“先 \texttt{readdir} 再对每个条目逐个 \texttt{getattr}”的往返。

除了缓存以外，FUSE 语义中还有两点与分离式架构较为相关。其一，\texttt{open} 在本文实现中不仅产生句柄，还负责把数据路由信息交给客户端。其二，\texttt{fsync} 目前只保证数据侧 \texttt{flush}，元数据依赖 RocksDB 的事务提交和 WAL 持久化，尚未实现严格的双阶段刷写。换言之，本文在 FUSE 这一层没有把内核接口原封不动地映射到远端服务，而是按元数据面和数据面分离后的职责边界，为每个接口给出了一个可落地的语义解释。

\subsection{gRPC 接口与模块边界}
\label{sec:grpc}

gRPC 在这条路径上不仅承担通信，也决定了哪些状态由哪个服务直接返回、哪些后续动作需要客户端继续协调，接口设计因此也是系统结构的一部分。本文在接口层面遵循两条原则：元数据操作和数据操作严格分离；一次 RPC 尽量返回下一阶段所需的全部信息，避免将简单操作拆成多轮往返。

据此，MetadataServer 的接口按职责分为四类：命名空间操作（\texttt{Lookup}/\texttt{Create}/\texttt{Rename}/\texttt{Readdir} 等）、属性操作（\texttt{GetAttr}/\texttt{SetAttr}）、句柄操作（\texttt{Open}/\texttt{Release}）以及写入后的元数据协调（\texttt{ReportWrite}）。DataServer 的接口则刻意收敛，只暴露 \texttt{Read}/\texttt{Write}/\texttt{Truncate}/\texttt{DeleteData} 等直接操作字节内容的调用。这样在接口层面即可判断“谁负责对象语义、谁负责数据内容”。

消息格式也服务于同一目标。元数据响应通常直接携带完整的 \texttt{FileAttr}、\texttt{DataLocation}、\texttt{purged\_inodes} 或 \texttt{needs\_truncate}，使客户端不必为完成一个上层操作而再发一轮探测 RPC。连接层面，当前原型采用“每客户端单 gRPC 通道、共享 HTTP/2 连接”的形式，主要是为了控制实现复杂度；这并非最终形态，但对本文所关心的元数据语义验证和原型级性能评估已经够用。
...
另一类失败来自普通的 RPC 或 I/O 调用。当前原型对此选择了较为保守的做法：客户端按 gRPC 返回的错误状态向上报相应的文件系统错误，不会在数据路径上维护待确认写队列，也没有实现基于 \texttt{inode + offset} 的幂等重发。也就是说，当前实现只做了“事务冲突自动重试”这一项，跨服务失败并不会被自动屏蔽，而是直接向应用暴露。
...
本文的研究重点是元数据面和数据面分离之后，如何通过键编码、事务控制、客户端协调和增量更新来优化元数据关键路径，而不是构建一套完整的大规模分布式控制面。相应地，第~\ref{sec:experiments}~章的实验也主要在当前边界内验证这套设计的正确性和性能特征，不声称系统已经覆盖多副本容错、跨节点故障转移或空间回收等更大范围的工程能力。
