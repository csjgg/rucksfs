...
经过 11 轮优化（6 轮合并、5 轮回退），RucksFS 修复了关键性能 bug 并优化了 RocksDB
写入路径。最终在 `-n 10000` 单线程 easy 模式下，**所有 6 个主要元数据操作都达到了
ext4 的 97%–117%**。

### 可靠基准数据（-n 10000，单线程，冷启动）

| 操作 | RucksFS (ops/s) | ext4 (ops/s) | 比值 |
|------|-----------------|-------------|------|
| create | 163,715 | 167,461 | 0.98x |
| stat | 1,143,912 | 1,178,303 | 0.97x |
| rename | 179,266 | 167,475 | **1.07x** |
| unlink | 232,007 | 197,725 | **1.17x** |
| mkdir | 76,242 | 76,464 | 1.00x |
| rmdir | 147,250 | 148,174 | 0.99x |

> readdir 在 -n 10000 下两者均为 ~560 ops/s，疑似 benchmark 工具测量逻辑问题，
> 不反映文件系统真实性能。
...
1. **unlink**：31 ops/s → 232,000 ops/s — 修复了同步零填充 64MB 的 bug
2. **多线程 create**：2T 从 5 ops/s → 37,596 ops/s — 消除后台 I/O 饱和
3. **所有 mutation 操作**：将每次操作的 2 次 RocksDB WAL 写入合并为 1 次

### 测量方法论反思

前 11 轮优化全部使用 `-n 100` 测量。这个规模存在严重问题：

- **总耗时 < 1ms**：100 个操作在优化后只需 0.5-5ms，时间精度不足
- **热运行偏差**：同一挂载点连续跑两次，第一次 create 21K，第二次 187K（差 9 倍）
- **虚高数字**：之前报告的 196K create ops/s 是热运行数据，冷启动只有 ~21K

**教训**：基准测试必须使用足够大的 -n（≥10000）确保总耗时 > 100ms，并在冷启动
条件下测量。`-n 100` 仅适合快速 A/B 对比（两次跑之间不重启），不适合报告绝对数值。
...
**关键结果**：unlink 31 → 5,180 ops/s（**163 倍**，-n 100 A/B 对比）
...
**关键结果**（-n 100 A/B 对比）：
- unlink：5,180 → 24,472 ops/s（**4.7 倍**）
- create 2T：5 → 37,596 ops/s（**修复多线程崩溃**）
- create hard 1T：0.88 → 15,965 ops/s（**修复 hard 模式**）
...
**关键结果**（-n 100 A/B 对比，相比 Round 5 基线）：
- create：15,434 → 196,978 ops/s（**12.8 倍**，注意热运行效应放大了数字）
- rename：18,836 → 204,849 ops/s（**10.9 倍**）
- unlink：24,472 → 231,673 ops/s（**9.5 倍**）
- mkdir：19,635 → 127,748 ops/s（**6.5 倍**）

> **注**：这些倍数包含了热运行偏差。实际在 -n 10000 冷启动下的改善应为 ~2 倍
> （从 2 次 WAL 写入减少到 1 次），这仍然是最重要的单次优化。
...
| 6 | 内联时间戳 delta | **合并** | 全部 mutation **~2x**（WAL 写入减半） |
...
- **可靠测量**：`-t 1 -n 10000`，冷启动（每次全新挂载），跑两次取平均
- **快速 A/B 对比**：`-t 1,2,4 -n 100`，同一挂载点连续跑（热运行），仅用于轮次间相对比较
- **验证**：每轮跑两次基准测试确认结果一致性
